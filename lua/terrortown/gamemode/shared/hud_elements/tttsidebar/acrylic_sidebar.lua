...
if CLIENT then
    ...
    local padding = 10
    local color_badstatus = Color(209, 43, 39)
    local color_goodstatus = Color(80, 173, 59)
    ...
    function HUDELEMENT:DrawIcon(curY, item)
        ...
        self:DrawBg(pos.x, curY, size.w, size.w, item.hud_color)
        self:DrawLines(pos.x, curY, size.w, size.w)
        ...
        if isfunction(item.DrawInfo) then
            ...
            if info then
                local infoW, infoH = draw.GetTextSize(info, "AcrylicItemInfo")
        ...
        if GAMEMODE.ShowScoreboard and GetConVar("ttt2_hud_enable_description"):GetBool() then
            ...
            local wrappedText =
                draw.GetWrappedText(translatedText, 285 * self.scale, "ItemInfo")
    ...
    function HUDELEMENT:Draw()
        ...
        for _, status in pairs(STATUS.active) do
            if status.type == "bad" then
                status.hud_color = ColorAlpha(color_badstatus, self.basecolor.a)
            end

            if status.type == "good" then
                status.hud_color = ColorAlpha(color_goodstatus, self.basecolor.a)
            end

            if status.type == "default" or (status.type == nil and status.hud_color == nil) then
                status.hud_color = Color(self.basecolor.r, self.basecolor.g, self.basecolor.b, self.basecolor.a)
            end
        ...
        for i = 1, #itms do
            ...
            item.hud_color = Color(self.basecolor.r, self.basecolor.g, self.basecolor.b, self.basecolor.a)
